...
export const sendMessageService = async (
  ...
  async function getAIResponse(chatId: string, userId: string) {
    const whoopAI = await UserModel.findOne({ isAI: true });
    if (!whoopAI) throw new NotFoundException("AI user not found");
    ...
    // return formattedMessages; // or replace with AI call result
  }
